...
func fade_and_change_scene(scene_path: String, fade_length: float = 0.3, fade_wait: float = 0.0):
	...
	tween.tween_property(color_rect, "color", Color(0.0, 0.0, 0.0, 1.0), fade_length).from(Color(0.0, 0.0, 0.0, 0.0)).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
...
#	yield(get_tree().create_timer(0.2), "timeout")
	...
	tween.tween_property(color_rect, "color", Color(0.0, 0.0, 0.0, 0.0), fade_length).from(Color(0.0, 0.0, 0.0, 1.0)).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
